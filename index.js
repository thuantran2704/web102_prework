...
const total_contri = GAMES_JSON.reduce((sum, game)=> sum + game.backers,0);
// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${total_contri.toLocaleString("en-US")}`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const total_raised = GAMES_JSON.reduce((sum, game) => sum + game.pledged ,0);
...
function filterUnfundedOnly(games) {
    ...
    const listUnfunded = games.filter(game =>game.pledged < game.goal);

    // use the function we previously created to add the unfunded games to the DOM
    return addGamesToPage(listUnfunded);
}
...
function filterFundedOnly(games) {
    ...
    const listFunded = games.filter(game =>game.pledged >= game.goal);

    // use the function we previously created to add unfunded games to the DOM
    return addGamesToPage(listFunded);
}
...
const totalUnfundedArr = GAMES_JSON.filter(game => game.pledged < game.goal);
const totalUnfundedNum = totalUnfundedArr.length;
const unfundedRaised = totalUnfundedArr.reduce((total,game) => total + game.pledged,0);
...
descriptionEle.innerHTML = totalUnfundedString;
...
let [mostFundedGame, secondMostFundedGame, ...remainingGames] = sortedGames;
